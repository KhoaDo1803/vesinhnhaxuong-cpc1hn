...
create policy "read records by department and area" on public.cleaning_records for select to authenticated using((select role from public.my_profile())='admin' or (department_code=(select department_code from public.my_profile()) and (((select role from public.my_profile())='ipc' and (cardinality((select area_codes from public.my_profile()))=0 or area_code=any((select area_codes from public.my_profile())))) or ((select role from public.my_profile())='factory_staff' and area_code=any((select area_codes from public.my_profile()))))));
create policy "factory creates records in scope" on public.cleaning_records for insert to authenticated with check(operator_id=auth.uid() and department_code=(select department_code from public.my_profile()) and area_code=any((select area_codes from public.my_profile())));
create policy "ipc reviews records in scope" on public.cleaning_records for update to authenticated using((select role from public.my_profile())='admin' or ((select role from public.my_profile())='ipc' and department_code=(select department_code from public.my_profile()) and (cardinality((select area_codes from public.my_profile()))=0 or area_code=any((select area_codes from public.my_profile()))))) with check((select role from public.my_profile()) in ('ipc','admin'));
